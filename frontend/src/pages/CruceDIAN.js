...
export default function CruceDIAN() {
  ...
  // Parse xlsx/xls/csv using SheetJS — runs 100% in the browser, no CORS issues
  const parseArchivoExcel = (arrayBuffer) => {
    const wb = XLSX.read(arrayBuffer, { type: 'array' });
    const ws = wb.Sheets[wb.SheetNames[0]];
    // Get all rows as arrays (raw values, skip header row 0)
    const rows = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
    const registros = [];
    // Row 0 = header, start from row 1
    for (let i = 1; i < rows.length; i++) {
      const cols = rows[i];
      // Column C = index 2 (numero de factura)
      const numero = String(cols[2] || '').trim();
      if (!numero) continue;
      registros.push({
        tipo:           String(cols[0] || '').trim(),
        cufe:           String(cols[1] || '').trim(),
        numero:         numero,
        fechaRecepcion: String(cols[3] || '').trim(),
        emisor:         String(cols[4] || '').trim(),
        iva:            String(cols[5] || '').trim(),
        total:          String(cols[6] || '').trim(),
        estado:         String(cols[7] || '').trim(),
        novedad:        String(cols[8] || '').trim(),
      });
    }
    return registros;
  };
  ...
  const procesarArchivo = async () => {
    ...
    try {
      ...
      const registrosDIAN = parseArchivoExcel(arrayBuffer);
      ...
      // Build lookup map: numero normalizado -> factura
      const mapaFacturas = {};
      facturas.forEach(f => {
